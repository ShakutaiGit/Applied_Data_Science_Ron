...
class lightGBM:
    ...
    def split_to_train_test(self):
        ...
        max_score = 0
        best_parameters = {'max_depth': {}, 'learning_rate': {}}
        for train_idx, val_idx in tqdm.tqdm(cv_outer.split(X_train, y_train)):
            ...
            gd_search = GridSearchCV(classifier, params, scoring='roc_auc', n_jobs=-1, cv=cv_inner).fit(train_data,
                                                                                                        train_target)
        ...
        model = LGBMClassifier(max_depth=mean_best_parameters['max_depth'],
                                   learning_rate=mean_best_parameters['learning_rate'], objective='binary').fit(X_train, y_train)
